...
// --- 게임 시작 ---
// 일부 함수 본문은 지면 관계상 생략하고 이전 코드와 동일하게 유지합니다.
// Full code for functions marked with /* ... */ is the same as the previous version.

[...document.querySelectorAll('script')].pop().previousSibling.textContent.split('\n').slice(10, -1).forEach(line => {
    if (line.includes('/* ... */')) {
        const funcName = line.split(' ')[1];
        switch (funcName) {
            case 'drawScores':
                window.drawScores = function() { ctx.font = "20px 'Press Start 2P'"; ctx.textAlign = 'center'; ctx.fillStyle = 'rgba(0, 0, 0, 0.4)'; ctx.fillRect(canvas.width / 2 - 160, 15, 320, 75); ctx.fillStyle = 'white'; ctx.fillText(`Sets: ${setScores.player} - ${setScores.opponent}`, canvas.width / 2, 45); ctx.font = "28px 'Press Start 2P'"; ctx.fillText(scores.player, canvas.width / 4, 75); ctx.fillText(scores.opponent, (canvas.width * 3) / 4, 75); ctx.textAlign = 'left'; };
                break;
            case 'updateAndDrawParticles':
                window.updateAndDrawParticles = function() { for (let i = particles.length - 1; i >= 0; i--) { let p = particles[i]; p.x += p.vx; p.y += p.vy; p.life--; if (p.life <= 0) particles.splice(i, 1); else { ctx.globalAlpha = p.life / 25; ctx.fillStyle = p.color; ctx.beginPath(); ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1.0; } } };
                break;
            case 'drawMenu':
                window.drawMenu = function() { drawCourt(); ctx.textAlign = 'center'; ctx.font = "32px 'Press Start 2P'"; ctx.fillStyle = 'white'; ctx.shadowColor = 'black'; ctx.shadowBlur = 5; ctx.fillText('Beach Volleyball', canvas.width / 2, 150); ctx.shadowBlur = 0; ctx.font = "24px 'Press Start 2P'"; ctx.fillStyle = menuSelection === 0 ? '#f1c40f' : 'white'; ctx.fillText('1 Player', canvas.width / 2, 250); ctx.fillStyle = menuSelection === 1 ? '#f1c40f' : 'white'; ctx.fillText('2 Players', canvas.width / 2, 300); ctx.font = "16px 'Press Start 2P'"; ctx.fillStyle = 'white'; ctx.fillText('Use Arrow Keys and Enter to Select', canvas.width / 2, 400); ctx.textAlign = 'left'; };
                break;
            case 'drawEndScreen':
                window.drawEndScreen = function() { ctx.fillStyle = 'rgba(0, 0, 0, 0.7)'; ctx.fillRect(0, 0, canvas.width, canvas.height); ctx.fillStyle = 'white'; ctx.font = "36px 'Press Start 2P'"; ctx.textAlign = 'center'; const winnerText = setScores.player >= WIN_SETS ? 'Player 1' : (gameMode === 1 ? 'Computer' : 'Player 2'); ctx.fillText(`${winnerText} Wins!`, canvas.width / 2, canvas.height / 2 - 40); ctx.font = "18px 'Press Start 2P'"; ctx.fillText('Press Enter to Return to Menu', canvas.width / 2, canvas.height / 2 + 20); ctx.textAlign = 'left'; };
                break;
            case 'isColliding':
                window.isColliding = function(c, r) { let dX = Math.abs(c.x - r.x - r.width / 2), dY = Math.abs(c.y - r.y - r.height / 2); if (dX > r.width / 2 + c.radius || dY > r.height / 2 + c.radius) return false; if (dX <= r.width / 2 || dY <= r.height / 2) return true; let corner = (dX - r.width / 2) ** 2 + (dY - r.height / 2) ** 2; return corner <= c.radius ** 2; };
                break;
            case 'checkSetWin':
                window.checkSetWin = function() { const pS = scores.player, oS = scores.opponent; let setW = null; if (pS >= WIN_SCORE && pS - oS >= 2) setW = 'player'; else if (oS >= WIN_SCORE && oS - pS >= 2) setW = 'opponent'; if (setW) { setScores[setW]++; if (setScores[setW] >= WIN_SETS) gameState = 'gameOver'; else { gameState = 'paused'; setTimeout(() => { scores.player = 0; scores.opponent = 0; gameState = 'play'; }, 1500); } } };
                break;
            case 'resetBall':
                window.resetBall = function(scoredBy) { if (gameState === 'paused' || gameState === 'gameOver') return; ball.vy = -5; ball.y = canvas.height / 2; ball.trail = []; if (scoredBy === 'player') { ball.x = (canvas.width * 3) / 4; ball.vx = -4; } else { ball.x = canvas.width / 4; ball.vx = 4; } };
                break;
        }
    }
});
