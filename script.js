...
// --- 초기화 함수 ---
function initialize() { initClouds(); initGameObjects(); gameLoop(); }
function initClouds() { clouds = [ { x: 100, y: 80, r: 25, speed: 0.1 }, { x: 350, y: 60, r: 35, speed: 0.15 }, { x: 500, y: 90, r: 30, speed: 0.12 } ]; }
...
function startNewMatch() { scores = { player: 0, opponent: 0 }; setScores = { player: 0, opponent: 0 }; initGameObjects(); gameState = 'play'; }
...
function gameLoop() {
    ...
    switch (gameState) {
        ...
        case 'play': runGameFrame(); break;
        case 'paused': runGameFrame(); break; // 세트 간 자동 일시정지
        case 'pause_menu': runGameFrame(); drawPauseMenu(); break; // 유저 일시정지
...
(function() {
    const functions = {
        ...
        updateAndDrawParticles: function() { for (let i = particles.length - 1; i >= 0; i--) { let p = particles[i]; p.x += p.vx; p.y += p.vy; p.life--; if (p.life <= 0) particles.splice(i, 1); else { ctx.globalAlpha = p.life / 25; ctx.fillStyle = p.color; ctx.beginPath(); ctx.arc(p.x, p.y, p.radius, 0, Math.PI * 2); ctx.fill(); ctx.globalAlpha = 1.0; } } },
        drawMenu: function() { drawCourt(); ctx.textAlign = 'center'; ctx.font = "32px 'Press Start 2P'"; ctx.fillStyle = 'white'; ctx.shadowColor = 'black'; ctx.shadowBlur = 5; ctx.fillText('Beach Volleyball', canvas.width / 2, 150); ctx.shadowBlur = 0; ctx.font = "24px 'Press Start 2P'"; ctx.fillStyle = menuSelection === 0 ? '#f1c40f' : 'white'; ctx.fillText('1 Player', canvas.width / 2, 250); ctx.fillStyle = menuSelection === 1 ? '#f1c40f' : 'white'; ctx.fillText('2 Players', canvas.width / 2, 300); ctx.font = "16px 'Press Start 2P'"; ctx.fillStyle = 'white'; ctx.fillText('Use Arrow Keys and Enter to Select', canvas.width / 2, 400); ctx.textAlign = 'left'; },
        drawEndScreen: function() { ctx.fillStyle = 'rgba(0, 0, 0, 0.7)'; ctx.fillRect(0, 0, canvas.width, canvas.height); ctx.fillStyle = 'white'; ctx.font = "36px 'Press Start 2P'"; ctx.textAlign = 'center'; const winnerText = setScores.player >= WIN_SETS ? 'Player 1' : (gameMode === 1 ? 'Computer' : 'Player 2'); ctx.fillText(`${winnerText} Wins!`, canvas.width / 2, canvas.height / 2 - 40); ctx.font = "18px 'Press Start 2P'"; ctx.fillText('Press Enter to Return to Menu', canvas.width / 2, canvas.height / 2 + 20); ctx.textAlign = 'left'; },
        createParticles: function(x, y, color) { for (let i = 0; i < 15; i++) { particles.push({ x, y, color, vx: (Math.random() - 0.5) * 4, vy: (Math.random() - 0.5) * 4, radius: Math.random() * 2.5 + 1, life: 25 }); } },
        applyPhysics: function(p) { p.vy += gravity; p.y += p.vy; p.x += p.vx; if (p.y + p.height > groundY) { p.y = groundY - p.height; p.vy = 0; p.onGround = true; } },
        updatePlayer1: function() { applyPhysics(player); if (player.x < 0) player.x = 0; if (player.x + player.width > netX) player.x = netX - player.width; },
        updatePlayer2: function() { applyPhysics(opponent); if (opponent.x < netX + netWidth) opponent.x = netX + netWidth; if (opponent.x + opponent.width > canvas.width) opponent.x = canvas.width - opponent.width; },
        updateAI: function() { if (ball.x > canvas.width / 2) { if (opponent.x + opponent.width / 2 < ball.x - 10) opponent.vx = 3; else if (opponent.x + opponent.width / 2 > ball.x + 10) opponent.vx = -3; else opponent.vx = 0; } if (ball.y < canvas.height / 1.5 && opponent.onGround && ball.x > canvas.width / 2) { opponent.vy = -9; opponent.onGround = false; } applyPhysics(opponent); if (opponent.x < netX + netWidth) opponent.x = netX + netWidth; if (opponent.x + opponent.width > canvas.width) opponent.x = canvas.width - opponent.width; },
        updateBall: function() { ball.vy += gravity / 2; ball.y += ball.vy; ball.x += ball.vx; if ((ball.x + ball.radius > canvas.width) || (ball.x - ball.radius < 0)) { ball.vx *= -1; createParticles(ball.x, ball.y, '#bdc3c7'); } if (ball.y - ball.radius < 0) { ball.vy *= -1; createParticles(ball.x, ball.y, '#bdc3c7'); } if (isColliding(ball, player)) handleCollision(ball, player); if (isColliding(ball, opponent)) handleCollision(ball, opponent); if (ball.x + ball.radius > netX && ball.x - ball.radius < netX + netWidth && ball.y + ball.radius > netY) { ball.vx *= -1.1; ball.x += ball.vx * 2; createParticles(ball.x, ball.y, 'white'); } if (ball.y + ball.radius > groundY) { createParticles(ball.x, groundY, '#FADDAA'); if (ball.x < canvas.width / 2) { scores.opponent++; checkSetWin(); resetBall('player'); } else { scores.player++; checkSetWin(); resetBall('opponent'); } } },
        handleCollision: function(b, p) { createParticles(b.x, b.y, p.color); let dx = b.x - (p.x + p.width / 2); b.vy = -6; b.vx = dx * 0.2; },
        isColliding: function(c, r) { let dX = Math.abs(c.x - r.x - r.width / 2), dY = Math.abs(c.y - r.y - r.height / 2); if (dX > r.width / 2 + c.radius || dY > r.height / 2 + c.radius) return false; if (dX <= r.width / 2 || dY <= r.height / 2) return true; let corner = (dX - r.width / 2) ** 2 + (dY - r.height / 2) ** 2; return corner <= c.radius ** 2; },
        checkSetWin: function() { const pS = scores.player, oS = scores.opponent; let setW = null; if (pS >= WIN_SCORE && pS - oS >= 2) setW = 'player'; else if (oS >= WIN_SCORE && oS - pS >= 2) setW = 'opponent'; if (setW) { setScores[setW]++; if (setScores[setW] >= WIN_SETS) gameState = 'gameOver'; else { gameState = 'paused'; setTimeout(() => { scores.player = 0; scores.opponent = 0; gameState = 'play'; }, 1500); } } },
        resetBall: function(scoredBy) { if (gameState === 'gameOver') return; ball.vy = -5; ball.y = canvas.height / 2; ball.trail = []; if (scoredBy === 'player') { ball.x = (canvas.width * 3) / 4; ball.vx = -4; } else { ball.x = canvas.width / 4; ball.vx = 4; } }
    };
    for(const name in functions) { window[name] = functions[name]; }
})();
